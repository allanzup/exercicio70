...
class ProdutoFragmentTest {
    ...
    fun testeToVerifyNotEmptyField() {
        val scenario = launchFragmentInContainer<ProdutoFragment>()
    ...
    @Test
    fun testToVerifyNameIsEmpty() {
        val scenario = launchFragmentInContainer<ProdutoFragment>()
        onView(withId(R.id.etDetalheProduto)).perform(typeText("produto1"))
        onView(withId(R.id.bvAdicionar)).perform(click())
        onView(withId(R.id.etNomeProduto)).check(matches(hasErrorText("Por favor preencha o campo de nome")))
        onView(withId(R.id.etDetalheProduto)).check(matches(hasNoErrorText()))
    }

    @Test
    fun testEmptyProductDetail(){
        val scenario = launchFragmentInContainer<ProdutoFragment>()
        onView(withId(R.id.etNomeProduto)).perform(typeText("produto1"))
        onView(withId(R.id.bvAdicionar)).perform(click())
